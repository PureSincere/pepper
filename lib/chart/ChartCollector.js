...
import BrokenLine from "./components/line/BrokenLine";
...
class ChartCollector {
  ...
  drawFps() {
    this.context.save();
    this.context.font = defaults.config.font;
    this.context.textAlign = "start";
    this.context.fillText(`FPS: ${this.fps}`, 20, defaults.config.fontSize);
  ...
  create(config) {
    ...
    switch (config.type) {
      case "bar": this.mainChart = Bar.Factory.create(this, mergeConfig(defaults.config, config)); break;
      case "brokenLine": this.mainChart = BrokenLine.Factory.create(this, mergeConfig(defaults.config, config)); break;
    }
    this.enableClick();
  ...
  handleClick(event) {
    ...
    utils.forEach(selectedCharts, chart => {
      if (chart.hasEventListener("click")) {
        chart.notifyEventListeners(Event.Factory.create(event, "click", chart));
      }
    });
  }
  ...
  handleMousemove(event) {
    ...
    utils.forEach(mousemoveCharts.selecteds, chart => {
      if (chart.hasEventListener("mousemove")) {
        chart.notifyEventListeners(Event.Factory.create(event, "mousemove", chart));
      }
    });
    ...
    utils.forEach(mouseoverCharts.selecteds, chart => {
      if (!chart.mouseInside) {
        ...
        if (chart.hasEventListener("mouseover")) {
          chart.notifyEventListeners(Event.Factory.create(event, "mouseover", chart));
        }
      }
    });
    ...
    utils.forEach(mouseoutCharts.unSelecteds, chart => {
      if (chart.mouseInside) {
        ...
        if (chart.hasEventListener("mouseout")) {
          chart.notifyEventListeners(Event.Factory.create(event, "mouseout", chart));
        }
      }
    });
  }
  ...
  windowToCanvas(x, y) {
    ...
    return Point.Factory.create(x - bbox.left, y - bbox.top);
  }
...
class ChartCollectorFactory {
  ...
  static create(canvas) {
    return new ChartCollector(canvas);
  }
}

ChartCollector.Factory = ChartCollectorFactory;
